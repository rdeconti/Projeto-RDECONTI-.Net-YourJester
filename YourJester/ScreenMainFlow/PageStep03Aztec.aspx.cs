...
/// Goal:           Based on user birthdate the portal shows to user data from Aztec oracle
/// *********************************************************************************************************************
...
{
    public partial class PageStep03Aztec : System.Web.UI.Page
    {
        ...
        protected void R0110DatabaseGetCode()
        {
            ...
            try
            {
                ...
                databaseCommand.CommandText = "SELECT SIGN FROM step03_astrology_aztec_calendar WHERE day_FROM <= @day and month_FROM <= @month and year_FROM <= @year and day_to >= @day and month_to >= @month and year_to >= @year";
        ...
        {
            imageCodeImage.ImageUrl = "../Images/aztec/" + keyCode + ".jpg";
        }
        ...
        protected void R0130DatabaseGetName()
        {
            ...
            try
            {
                ...
                databaseCommand.CommandText = "SELECT NAME FROM step03_astrology_aztec_names WHERE code=@code and language=@language";
        ...
        protected void R0140DatabaseGetDescription()
        {
            ...
            try
            {
                ...
                databaseCommand.CommandText = "SELECT DESCRIPTION FROM step03_astrology_aztec_description WHERE sign=@code and language=@language";
                databaseCommand.Parameters.AddWithValue("@code", keyCode);
                databaseCommand.Parameters.AddWithValue("@language", userLanguage);
                databaseCommand.Connection = databaseConnection;
        ...
        protected void R0150DatabaseGetPositiveCharacteristics()
        {
            ...
            try
            {
                ...
                databaseCommand.CommandText = "SELECT CHARACTERISTIC FROM step03_astrology_aztec_characteristics WHERE code=@code and type=@type";
                databaseCommand.Parameters.AddWithValue("@code", keyCode);
            ...
            try
            {
                ...
                if (databaseContainer.Read())
                {
                    if ((databaseContainer.GetString("phase01") == "N"))
                    {
                        ...
                        databaseCommand01.CommandText = "UPDATE users_step03_results SET test_start=@test_start, positive=@positive, negative=@negative, neutral=@neutral, questions=@questions, phase01=@phase01 WHERE userCode=@userCode";
                ...
                else
                {
                    ...
                    databaseCommand02.CommandText = "INSERT INTO users_step03_results (userCode, test_start, test_finish, positive, negative, neutral, questions, phase01) VALUES (@userCode, @test_start, @test_finish, @positive, @negative, @neutral, @questions, @phase01)";
        ...
        protected void R0310DatabaseUpdateUserProgress()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    databaseCommand02.Parameters.AddWithValue("@userCode", userCode);
                    databaseCommand02.Parameters.AddWithValue("@phase_level_1", "03");
                    ...
                    databaseCommand02.Parameters.AddWithValue("@questions", counterCharacteristicsQuestions);
                    databaseCommand02.Parameters.AddWithValue("@answers", counterCharacteristicsQuestions);
                    databaseCommand02.Parameters.AddWithValue("@agree", counterCharacteristicsQuestions);
                    databaseCommand02.Connection = databaseConnection;
                    databaseCommand02.ExecuteNonQuery();
                }
            }
