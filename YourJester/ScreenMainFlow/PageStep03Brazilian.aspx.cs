...
/// Goal:           Based on user birthdate the portal shows to user data from Brazilian oracle
/// *********************************************************************************************************************
...
{
    public partial class PageStep03Brazilian : System.Web.UI.Page
    {
        ...
        protected void R0110DatabaseGetCode()
        {
            ...
            try
            {
                ...
                databaseCommand.CommandText = "SELECT SIGN FROM step03_astrology_brazilian_calendar WHERE day_FROM <= @day and month_FROM <= @month and day_to >= @day and month_to >= @month";
        ...
        {
            imageCodeImage.ImageUrl = "../Images/brazilian/" + keyCode + ".jpg";
        }
        ...
        protected void R0130DatabaseGetName()
        {
            ...
            try
            {
                ...
                databaseCommand.CommandText = "SELECT NAME FROM step03_astrology_brazilian_names WHERE code=@code and language=@language";
        ...
        protected void R0140DatabaseGetDescription()
        {
            ...
            try
            {
                ...
                databaseCommand.CommandText = "SELECT DESCRIPTION FROM step03_astrology_brazilian_description WHERE sign=@code and language=@language";
                databaseCommand.Parameters.AddWithValue("@code", keyCode);
                databaseCommand.Parameters.AddWithValue("@language", userLanguage);
                databaseCommand.Connection = databaseConnection;
        ...
        protected void R0150DatabaseGetPositiveCharacteristics()
        {
            ...
            try
            {
                ...
                databaseCommand.CommandText = "SELECT CHARACTERISTIC FROM step03_astrology_brazilian_characteristics WHERE code=@code and type=@type";
                databaseCommand.Parameters.AddWithValue("@code", keyCode);
            ...
            try
            {
                ...
                if (databaseContainer.Read())
                {
                    if ((databaseContainer.GetString("phase02") == "N"))
                    {
                        ...
                        databaseCommand01.CommandText = "UPDATE users_step03_results SET test_start=@test_start, positive=@positive, negative=@negative, neutral=@neutral, questions=@questions, phase02=@phase02 WHERE userCode=@userCode";
                ...
                else
                {
                    ...
                    databaseCommand02.CommandText = "INSERT INTO users_step03_results (userCode, test_start, test_finish, positive, negative, neutral, questions, phase02) VALUES (@userCode, @test_start, @test_finish, @positive, @negative, @neutral, @questions, @phase02)";
        ...
        protected void R0310DatabaseUpdateUserProgress()
        {
            ...
            try
            {
                ...
                databaseCommand.Parameters.AddWithValue("@questions", counterCharacteristicsQuestions);
                databaseCommand.Parameters.AddWithValue("@answers", counterCharacteristicsQuestions);
                databaseCommand.Parameters.AddWithValue("@agree", counterCharacteristicsQuestions);
                databaseCommand.Connection = databaseConnection;
